...
export async function listEventRegistrationsController(req: Request, res: Response) {
  const userId = req.authUserId;
  if (!userId) return res.status(401).json({ message: "Unauthorized" });

  try {
    const registrations = await listEventRegistrations(req.params.id, userId);
    if (registrations === null) return res.status(404).json({ message: "Event not found" });
...
export async function deleteEventRegistrationController(req: Request, res: Response) {
  const userId = req.authUserId;
  if (!userId) return res.status(401).json({ message: "Unauthorized" });

  try {
    const deleted = await deleteEventRegistration(req.params.id, req.params.registrationId, userId);
    if (deleted === null) return res.status(404).json({ message: "Not found" });
    return res.status(204).send();
  } catch (error) {
    if (error instanceof Error && error.message === "FORBIDDEN") {
      return res.status(403).json({ message: "Forbidden" });
...
export async function issueCertificateController(req: Request, res: Response) {
  const userId = req.authUserId;
  if (!userId) return res.status(401).json({ message: "Unauthorized" });

  try {
    const result = await issueRegistrationCertificate(
      req.params.id,
      req.params.registrationId,
      userId
    );
    if (result === null) return res.status(404).json({ message: "Registration not found" });
...
export async function revokeCertificateController(req: Request, res: Response) {
  const userId = req.authUserId;
  if (!userId) return res.status(401).json({ message: "Unauthorized" });

  try {
    const result = await revokeRegistrationCertificate(
      req.params.id,
      req.params.registrationId,
      userId
    );
