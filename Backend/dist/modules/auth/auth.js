...
const getSmtpTransporter = async () => {
    ...
    if (isGmailHost(smtpHost) && smtpPass.length !== 16 && !hasLoggedGmailPasswordFormatWarning) {
        ...
        console.error(`[auth] SMTP_PASS looks invalid for Gmail (normalized length: ${smtpPass.length}). Gmail App Password should be 16 characters.`);
    }
    ...
    try {
        ...
        if (isSmtpError(error) && error.code === "EAUTH") {
            if (env.SMTP_HOST && isGmailHost(env.SMTP_HOST) && error.responseCode === 535) {
                console.error("[auth] Gmail SMTP rejected credentials (535). Use a Google App Password (16 chars), not your account password.");
