...
export default function UpdateEventsPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold tracking-tight">Update Events</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Select an event to edit its details and change its image.
        </p>
      </div>
        ...
        <div className="space-y-3">
          {events.map((event) => {
            ...
            return isCompleted ? (
              <div
                key={event.id}
                className="group flex items-center gap-4 rounded-xl border bg-background px-4 py-3 opacity-70 cursor-not-allowed"
              >
                {content}
              </div>
            ) : (
              <Link
                key={event.id}
                href={`/dashboard/organizer/update-events/${event.id}`}
